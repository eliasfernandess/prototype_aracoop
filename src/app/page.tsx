    ...
    <main className="min-h-screen" style={{ background: '#F4F6F5' }}>
      ...
      >
        ...
        <div className="flex items-center gap-1.5 mb-1">
          <h1 className="text-white font-bold text-xl">Sicoob Aracoop</h1>
          <CheckCircle size={18} className="text-white/80" />
        </div>
        <p className="text-white/60 text-sm mb-4">@sicoobaracoop</p>

        {/* Bio */}
        <p className="text-white/80 text-sm text-center max-w-xs leading-relaxed mb-5">{bio}</p>
        ...
        <div className="flex items-center gap-3">
          ...
          <a
            href="https://instagram.com/sicoobaracoop"
            target="_blank"
            rel="noopener noreferrer"
            className="w-10 h-10 rounded-full flex items-center justify-center transition-colors"
            style={{ background: 'rgba(255,255,255,0.15)' }}
        ...
        <div className="flex flex-col gap-3">
          ...
          {links.map((link) => (
            <LinkButton
              ...
              url={buildUtmUrl(link.urlDestino, {
                utmSource: link.utmSource,
                utmMedium: link.utmMedium,
                utmCampaign: link.utmCampaign,
                utmContent: link.utmContent,
              })}
              campanha={link.utmCampaign}
              icone={link.icone}
              corIcone={link.corIcone}
            />
          ))}
        </div>
